...
# ------------------------------------------------------------------------ cars
REFERENCE_METRES = 18.425   # the driving car. EVERY car takes its cross-section
                            # from this one. MEASURED: es.wikipedia.org, "18.425
                            # para los coches motores con cabina".
...
#
# The 9000 is a different animal. Its cars are 18.425 m (the two with cabs) and
# 16.880 m (the other four) - a difference of NINE PER CENT, far below the size of
# one carunit. They round to the same length whatever you do. So here, equal
# lengths are what the prototype actually has, the joints close, and nothing is
# being papered over.
#
...
def livery_texture(car, decals=True):
    ...
    # ---- bodywork. y=0 is the BOTTOM. Ivory - but SHADED FOR VOLUME now, not a flat
    # slab. The old version only darkened the sill; a real steel side also has a
    # highlight up where the sun catches the shoulder, and that top-to-bottom gradient
    # is most of what makes pak128's own stock look rounded. This is the pass the 2000A
    # got and the 9000 did not - it is why the 9000 read as a sticker beside it.
    rect(0.00, 0.21, 1.00, 0.86, BODY)
    rect(0.00, 0.21, 1.00, 0.30, BODY_SH)      # shadow down along the sill
    rect(0.00, 0.47, 1.00, 0.55, BODY_HI)      # highlight up under the window band
    rect(0.00, 0.815, 1.00, 0.862, BODY_HI)    # the catch-light strip under the blue
    ...
    # ---- the window band, stopping short of the blue. Now FRAMED top and bottom and
    # given a reflection, so the aluminium ribbon reads as a row of windows with depth
    # instead of a flat dark smear. Kept continuous (this train IS a ribbon, unlike the
    # 2000A's punched holes) - the structure comes from rails and reflections, not from
    # cutting it into separate panes.
    rect(0.00, 0.55, 1.00, 0.815, GLASS)
    rect(0.00, 0.800, 1.00, 0.815, EQUIP_DET)      # header rail along the top
    rect(0.00, 0.550, 1.00, 0.562, DARK)           # sill rail along the bottom
    ...
    while x < 0.95:
        ...
        rect(x, 0.585, x + w, 0.78, LIT_PANE if lit else GLASS, light=lit)
        rect(x, 0.585, x + w, 0.60, GLASS_DEEP)        # a shadow under the pane
        rect(x, 0.760, x + w, 0.78, GLASS_HI)          # a reflection along the top
        rect(x - 0.007, 0.585, x, 0.78, EQUIP_DET)     # the mullion, left of the pane
        x += 0.095
        i += 1
...
def build_cab(b, mats):
    ...
    nose.data.materials.append(mats["blue"])       # the 9000's nose is BLUE
...
def materials():
    m = {
        "body": rig.make_paint_material(bpy, BODY, "m9k_body"),
        "blue": rig.make_paint_material(bpy, BLUE, "m9k_blue"),
        "red": rig.make_paint_material(bpy, RED, "m9k_red"),
        "glass": rig.make_paint_material(bpy, GLASS, "m9k_glass"),
        "roof": rig.make_paint_material(bpy, ROOF, "m9k_roof"),
        "equip": rig.make_paint_material(bpy, EQUIP, "m9k_equip"),
        "equip_det": rig.make_paint_material(bpy, EQUIP_DET, "m9k_equip_det"),
        "under": rig.make_paint_material(bpy, UNDER, "m9k_under"),
        "bogie": rig.make_paint_material(bpy, BOGIE, "m9k_bogie"),
        "dark": rig.make_paint_material(bpy, DARK, "m9k_dark"),
        "deep": rig.make_paint_material(bpy, DEEP, "m9k_deep"),
        "panto": rig.make_paint_material(bpy, PANTO_METAL, "m9k_panto"),
        "insulator": rig.make_paint_material(bpy, PANTO_INSULATOR, "m9k_insul"),
        # exact, unlit, or the engine will not recognise them
        "headlamp": rig.make_special_color_material(bpy, HEADLIGHT, "m9k_head"),
        "taillamp": rig.make_special_color_material(bpy, TAILLIGHT, "m9k_tail"),
        "player": rig.make_special_color_material(bpy, PLAYER, "m9k_player"),
    }
    return m
