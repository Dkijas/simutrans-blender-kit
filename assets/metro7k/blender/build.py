...
def build_one(metro7k, car, constraints, install=True):
    ...
    # (1260) and per-car length (17.83 m, which is just 107/6) were wrong, could not
    ...
    # totals. The 9000 shipped a split summing to 186 against a measured 178 back
...
def main():
    ...
    if which == "all":
        ...
        # spec at all, so the arithmetic already holds. The 9000 that shipped 186
        ...
        got = [c.key for c in metro7k.UNIT]
        check("the module builds exactly the unit the spec describes, in order",
              got == want, "spec: %s\n         module: %s" % (want, got))

        check("and their roles spell the declared formation",
              "-".join(c.role for c in metro7k.UNIT) == spec.formation,
              "%s vs %s" % ("-".join(c.role for c in metro7k.UNIT), spec.formation))
