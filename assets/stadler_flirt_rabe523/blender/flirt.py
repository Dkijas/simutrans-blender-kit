...
# v0.1.1: length 8 -> 12. At ~25 m/tile a real 18.5 m FLIRT module is 0.75 tile =
# 12/16, not 0.5 tile (8/16). With length=8 each car was half a tile and the visual
# body (~0.5 tile) left big gaps between cars - worst on the foreshortened diagonals
# - because the whole four-car unit was only 2 tiles (~50 m) instead of ~74 m. All
# four cars share ONE length, so straight joints still butt (the civia_465 rule);
# only the value changes, matched to the real car so the cars sit close.
PAK128_VEHICLE_LENGTH = 12  # every car, equal (see the note above)
...
def livery_texture(car, decals=True):
    ...
    # ---- the doors: RED, cutting the band clean through (SBB). The bottom stops at
    # DOOR_BOT, ABOVE the grey sill band: earlier the doors reached y=0.15 (the very
    # skirt) so in low isometric views the door red pooled into a red slab under the
    # car and, at the ends, the red patches/triangles reported as "foreign red". The
    # sill (grey band + dark underframe) is now kept red-free.
    DOOR_BOT = 0.30
    for centre in car.doors:
        rect(centre - 0.048, DOOR_BOT, centre + 0.048, 0.84, RED)
        rect(centre - 0.048, DOOR_BOT, centre + 0.048, DOOR_BOT + 0.06, RED_SH)  # shadow low
        rect(centre - 0.048, 0.80, centre + 0.048, 0.84, RED_HI)   # highlight high
        rect(centre - 0.050, DOOR_BOT, centre - 0.048, 0.84, DARK)     # frames
        rect(centre + 0.048, DOOR_BOT, centre + 0.050, 0.84, DARK)
        rect(centre - 0.002, DOOR_BOT, centre + 0.002, 0.84, DARK)     # leaf split
...
def build_roof(b, mats, panto):
    ...
    # roof equipment. On the panto car the centre is left clear so the pantograph
    # reads on its own (the other cars keep their boxes, differentiated by function).
    roof_positions = (-0.30, 0.24) if panto else (-0.30, -0.05, 0.22)
    for ax in roof_positions:
    ...
    if panto:
        # A clearly-read folded pantograph, centred: four corner insulators, a base
        # frame, two rising arms, and a WIDE bow standing proud of the roof - the
        # silhouette that says "pantograph" even in the diagonal headings. Dark metal
        # on the light-grey roof gives the contrast v0.1 lacked.
        for sy in (-1, 1):
            for sx2 in (-1, 1):
                box((sx2 * b.length * 0.12, sy * b.width * 0.24,
                     b.top + b.height * 0.07),
                    (b.width * 0.11, b.width * 0.11, b.height * 0.06),
                    mats["insulator"], name="panto_insulator")
        box((0, 0, b.top + b.height * 0.11),
            (b.length * 0.34, b.width * 0.64, b.height * 0.02),
            mats["panto"], name="panto_base")
        for ax2 in (-1, 1):                     # two rising arms
            box((ax2 * b.length * 0.09, 0, b.top + b.height * 0.20),
                (b.length * 0.035, b.width * 0.10, b.height * 0.20),
                mats["panto"], name="panto_arm")
        box((0, 0, b.top + b.height * 0.30),    # the wide bow, proud of the roof
            (b.length * 0.11, b.width * 0.76, b.height * 0.028),
            mats["panto"], name="panto_bow")


def build_cab(b, mats):
    """The nose: a PROJECTED red bulb that caps up into the roof (less of a vertical
    wall than v0.1), a wide wrapping windscreen, lamps set into the front, and a dark
    lower apron so the red front does not pool into a red slab at the sill in low iso
    views. BOTH lamps on the front (the engine cannot switch them by convoy position).
    Moderate change only - same silhouette family, still legible at 128 px."""
    # more projected and a touch taller, shifted forward, so the crown meets the roof
    bpy.ops.mesh.primitive_uv_sphere_add(segments=28, ring_count=18, radius=0.5,
                                         location=(b.half + b.width * 0.05, 0,
                                                   b.mid + b.height * 0.03))
    nose = bpy.context.active_object
    nose.name = "nose"
    nose.scale = (b.width * 0.98, b.width, b.height * 1.04)
    nose.data.materials.append(mats["red"])

    # a wide, wrapping windscreen set into the upper front
    bpy.ops.mesh.primitive_uv_sphere_add(segments=24, ring_count=14, radius=0.5,
                                         location=(b.half + b.width * 0.10, 0,
                                                   b.mid + b.height * 0.20))
    screen = bpy.context.active_object
    screen.name = "windscreen"
    screen.scale = (b.width * 0.92, b.width * 0.94, b.height * 0.46)
    screen.data.materials.append(mats["glass"])

    # lamps set into the red front, smaller and better integrated than v0.1
    for sy in (-1, 1):
        box((b.half + b.width * 0.34, sy * b.width * 0.28,
             b.floor + b.height * 0.30),
            (b.width * 0.09, b.width * 0.13, b.height * 0.07), mats["headlamp"],
            name="headlight")
        box((b.half + b.width * 0.34, sy * b.width * 0.28,
             b.floor + b.height * 0.19),
            (b.width * 0.08, b.width * 0.11, b.height * 0.045), mats["taillamp"],
            name="taillight")

    # dark lower apron: keeps the red front off the sill (no red slab at the base in
    # low iso views) and reads as the bumper/coupler
    box((b.half + b.width * 0.16, 0, b.floor * 0.62),
        (b.width * 0.66, b.width * 0.92, b.floor * 1.30), mats["deep"],
        name="nose_skirt")
    box((b.half + b.width * 0.30, 0, b.floor * 0.32),
        (b.width * 0.16, b.width * 0.30, b.floor * 0.58), mats["dark"],
        name="coupler")
