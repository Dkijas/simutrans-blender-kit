"""Build the Civia S/465 THROUGH THE ADD-ON, not around it.

    blender --background --factory-startup --python assets/civia_465/blender/build.py \
            -- cab_a                 # one car (the prototype)
    blender ... -- all               # the whole five-car unit, with couplings
...
_PROJ = os.path.dirname(_HERE)                       # assets/civia_465
...
def couplings(cars):
    ...
    possible successor, the depot will assemble the whole five-car unit from a
...
def build_one(civia465, car, constraints, install=True):
    from simutrans_blender_kit.addon import rig      # the INSTALLED copy

    body = civia465.build_car(car)
    ...
    p.basename = car.key
    p.obj_name = car.name
    p.author = civia465.AUTHOR
    p.out_dir = SPRITES

    # --- the numbers. Measured, sourced, or provisional - and reports/ says which.
    p.waytype = "track"
    p.engine_type = "electric"
    p.speed = 120                     # measured: Wikipedia, series 465
    p.power = car.kilowatts           # 2200 kW over the unit, split across the
    p.weight = car.tonnes             # powered cars; 157.3 t over the unit
    p.length = car.length             # from the measured metres
    p.freight = "Passagiere"
    p.payload = car.seats             # 997 over the unit
    p.intro_year = 2004               # sourced: the 465 entered service in 2004
    p.cost = 1800000 if car.cab else 900000        # PROVISIONAL, see TODO_BALANCE
    p.runningcost = 190 if car.cab else 95         # PROVISIONAL
    ...
    check("%s: the windows really are the engine's window colour" % car.key,
          wanted.get(civia465.LIT_PANE, 0) > 30,
          "%d px" % wanted.get(civia465.LIT_PANE, 0))
    if car.cab:
        check("%s: the headlights are the engine's light colour" % car.key,
              wanted.get(civia465.HEADLIGHT, 0) > 4,
              "%d px" % wanted.get(civia465.HEADLIGHT, 0))
        check("%s: the tail lights are the engine's red" % car.key,
              wanted.get(civia465.TAILLIGHT, 0) > 4,
              "%d px" % wanted.get(civia465.TAILLIGHT, 0))
...
def main():
    ...
    import civia465                              # noqa: E402  (after the install)

    if which == "all":
        cars = list(civia465.UNIT)
        cons = couplings(cars)
    else:
        ...
        cars = [c for c in civia465.UNIT if c.key.endswith(which)]
        if not cars:
            raise SystemExit("unknown car: %s" % which)
        cons = {}

    print("\n=== Civia S/465 for pak128: %s ===\n"
          % ", ".join(c.key for c in cars))
    for car in cars:
        print("--- %s (%s, %.2f m -> length %d)"
              % (car.key, car.name, car.metres, car.length))
        build_one(civia465, car, cons)

    print("\nout: %s" % _PROJ)
    if FAILED:
        print("\nCIVIA465_FAILED: %s" % ", ".join(FAILED))
        sys.exit(1)
    print("\nCIVIA465_OK")
